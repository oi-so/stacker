...
from PySide6.QtGui import QImage, QMouseEvent, QNativeGestureEvent, QPainter, QPixmap
...
class ImageViewer(QGraphicsView):
    ...
    def _to_qimage(self, image: np.ndarray) -> QImage:
        ...
        sample_step = max(1, int(np.sqrt(arr.size / 500_000)))
        sample = arr[::sample_step, ::sample_step]
        finite = sample[np.isfinite(sample)]
        if finite.size == 0:
            arr = np.zeros_like(arr, dtype=np.float32)
        else:
            bg = np.median(finite)
            sigma = max(float(np.std(finite)), 1e-8)
            stretch = 5
        ...
        arr8 = np.ascontiguousarray((arr * 255).round().astype(np.uint8))
        if arr8.ndim == 2:
            h, w = arr8.shape
        ...
        if arr8.shape[2] == 1:
            ...
            return QImage(arr8.data, w, h, w, QImage.Format.Format_Grayscale8).copy()
        if arr8.shape[2] > 3:
            arr8 = np.ascontiguousarray(arr8[..., :3])
        h, w, _ = arr8.shape
